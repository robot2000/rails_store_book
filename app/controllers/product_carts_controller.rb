class ProductCartsController < ApplicationController
  ...
  def create
    ...
    respond_to do |format|
      if @product_cart.save
        format.html { redirect_to @product_cart.cart }
  ...
  #   respond_to do |format|
  #     format.html { redirect_to product_carts_url, notice: 'Product was destroyed.' }
  ...
  def destroy
    ...
    respond_to do |format|
      format.html { redirect_to(@product_cart.cart, :notice => 'Item Removed' )}
      format.xml  { head :ok }
    end
  end
